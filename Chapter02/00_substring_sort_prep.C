// Common code for constructing strings for benchmarks.
// Not used directly but included from other files.
    //constexpr unsigned int L = 1 << 24, N = 1 << 20; // Use with options A, B
    constexpr unsigned int L = 1 << 18, N = 1 << 14;  // Use with option C
    //constexpr unsigned int L = 1 << 19, N = 1 << 15;  // Use with option C, takes slightly longer
