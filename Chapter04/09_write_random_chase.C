...
void BM_write_rand_chase(benchmark::State& state) {
    const size_t size = state.range(0);
    const size_t N = size/sizeof(node_t);
    if ((N % 32) != 0) abort();
    std::vector<node_t> data(N);
    data_t fill; ::memset(&fill, 0xab, sizeof(fill));
    ...
    for (auto _ : state) {
        ...
        for (size_t i = 0; i != N; i += 32) {
            REPEAT(cur->d = fill; cur = cur->next;)
        }
...
void BM_write_rand_index(benchmark::State& state) {
    const size_t size = state.range(0);
    const size_t N = size/sizeof(node_t);
    if ((N % 32) != 0) abort();
...
void BM_write_rand_ptr(benchmark::State& state) {
    const size_t size = state.range(0);
    const size_t N = size/sizeof(node_t);
    if ((N % 32) != 0) abort();
    std::vector<data_t> data(N);
...
BENCHMARK(BM_write_rand_chase) ARGS;
BENCHMARK(BM_write_rand_index) ARGS;
BENCHMARK(BM_write_rand_ptr) ARGS;
