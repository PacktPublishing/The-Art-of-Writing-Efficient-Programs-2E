...
class smart_ptr_base {
 ...
 public:
  ...
  [[nodiscard]] long use_count() const noexcept {
    return get_unmarked_control_block() ? get_unmarked_control_block()->get_use_count() : 0;
  }
 ...
 protected:
  ...
  [[nodiscard]] control_block_base* get_unmarked_control_block() const noexcept {
    return reinterpret_cast<control_block_base*>(reinterpret_cast<uintptr_t>(control_block) & ~1ULL);
  }

  void increment_strong() const noexcept {
    if (auto cb = get_unmarked_control_block()) {
      cb->increment_strong_count();
    }
  }
  
  [[nodiscard]] bool increment_if_nonzero() const noexcept {
    if (auto cb = get_unmarked_control_block()) {
      return cb->increment_strong_count_if_nonzero();
    }
    return false;
  }
  ...
  void decrement_strong() noexcept {
    if (auto cb = get_unmarked_control_block()) {
      cb->decrement_strong_count();
    }
  }
  
  void increment_weak() const noexcept {
    if (auto cb = get_unmarked_control_block()) {
      cb->increment_weak_count();
    }
  }
  
  void decrement_weak() noexcept {
    if (auto cb = get_unmarked_control_block()) {
      cb->decrement_weak_count();
    }
  }
