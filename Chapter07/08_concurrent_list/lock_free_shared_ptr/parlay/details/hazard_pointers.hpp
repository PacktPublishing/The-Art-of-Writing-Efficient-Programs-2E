...
#ifndef PARLAY_PREFETCH
#define PARLAY_PREFETCH(addr, rw, locality) __builtin_prefetch(addr, rw, locality)
#endif
#ifndef PARLAY_NOINLINE
#define PARLAY_NOINLINE __attribute__((noinline))
#endif
...
public:
  ...
  U protect(const Atomic<U> &src, F &&f) {
    ...
    while (true) {
      ...
      std::atomic_thread_fence(std::memory_order_seq_cst);    /*  Fast-side fence  */
