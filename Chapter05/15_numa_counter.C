...
std::atomic<size_t>& x0 = (new value_t)->value;
std::atomic<size_t>& x1 = (new value_t)->value;
...
void BM_increment_node0(benchmark::State& state) {
    ...
    for (auto _ : state) {
        REPEAT(x0.fetch_add(1, std::memory_order_relaxed);)
    }
...
void BM_increment_per_node(benchmark::State& state) {
    if (numnodes <= 1) state.SkipWithError("No NUMA nodes");
    const size_t node = state.thread_index() % numnodes;
    if (pthread_setaffinity_np(pthread_self(), sizeof(cpu_set_t), &node_cpu_mask[node]) != 0) { std::cout << "Pinning failed on line " << __LINE__ << std::endl; abort(); }
    auto& x = node == 0 ? x0 : x1;
...
#define ARG UseRealTime()->Threads(numcpu)

BENCHMARK(BM_increment_node0)->ARG;
BENCHMARK(BM_increment_node_pinned_threads)->ARG;
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG;
BENCHMARK(BM_increment_per_node)->ARG;
