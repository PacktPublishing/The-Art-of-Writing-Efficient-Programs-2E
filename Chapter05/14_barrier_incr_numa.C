...
void BM_increment_node0(benchmark::State& state) {
    ...
    for (auto _ : state) {
        for (size_t i = 0; i != N; ++i) {
            a[i].value.fetch_add(1, std::memory_order_relaxed);
        }
    }
...
#define ARG2(N) UseRealTime()->Threads(numcpu/N)->Arg(2)

BENCHMARK(BM_increment_node0)->ARG2(1);
BENCHMARK(BM_increment_node_pinned_threads)->ARG2(1);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG2(1);

BENCHMARK(BM_increment_node0)->ARG2(2);
BENCHMARK(BM_increment_node_pinned_threads)->ARG2(2);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG2(2);

BENCHMARK(BM_increment_node0)->ARG2(4);
BENCHMARK(BM_increment_node_pinned_threads)->ARG2(4);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG2(4);

#define ARG32(N) UseRealTime()->Threads(numcpu/N)->Arg(32)

BENCHMARK(BM_increment_node0)->ARG32(1);
BENCHMARK(BM_increment_node_pinned_threads)->ARG32(1);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG32(1);

BENCHMARK(BM_increment_node0)->ARG32(2);
BENCHMARK(BM_increment_node_pinned_threads)->ARG32(2);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG32(2);

BENCHMARK(BM_increment_node0)->ARG32(4);
BENCHMARK(BM_increment_node_pinned_threads)->ARG32(4);
BENCHMARK(BM_increment_cpu_pinned_threads)->ARG32(4);
